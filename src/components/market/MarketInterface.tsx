...
export function MarketInterface() {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto p-4 sm:p-6 lg:p-8 text-white pb-20">
      
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-black mb-2">Perpetual Markets</h1>
          <p className="text-zinc-400">Discover, track, and trade regional and global crypto assets.</p>
        </div>
        <div className="relative w-full md:w-72 shrink-0">
          ...
          <input 
            ...
            className="w-full bg-[#1c1c1f] border border-zinc-800 rounded-full py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:border-blue-500 transition-colors placeholder:text-zinc-600"
          />
        </div>
      </div>

      {/* Info Bar */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-[#111114] border border-zinc-800/50 rounded-xl mb-6 gap-4">
        <div className="flex flex-wrap items-center gap-3 text-sm">
          <span className="text-zinc-400">Last updated: <span className="text-white font-mono">{lastUpdated || '--:--:--'}</span></span>
          <span className="text-zinc-700 hidden sm:inline">|</span>
          <div className="flex items-center gap-1.5 text-cyan-400 font-medium">
            <div className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />
            WebSocket Live Feed
          </div>
          <div className="flex items-center gap-1.5 text-green-500 border border-green-500/20 bg-green-500/10 px-2 py-0.5 rounded-full text-xs font-semibold">
            ✓ Live Rates
          </div>
        </div>
        <button 
          onClick={fetchAssets}
          className="flex items-center gap-2 border border-zinc-700 hover:border-zinc-500 bg-[#1c1c1f] hover:bg-zinc-800 px-4 py-2 rounded-lg text-sm font-semibold transition-colors text-cyan-400 shrink-0"
        >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors border ${
              activeTab === tab.name 
                ...
                : 'bg-[#111114] text-zinc-400 border-zinc-800 hover:bg-zinc-800 hover:text-white'
            }`}
          >
      ...
      {/* Table */}
      <div className="bg-[#111114] border border-zinc-800/50 rounded-xl overflow-hidden overflow-x-auto">
        <table className="w-full text-left min-w-[800px]">
          <thead>
            <tr className="text-xs font-bold text-zinc-500 tracking-wider uppercase border-b border-zinc-800/50 bg-[#0a0a0c]/50">
              ...
              displayAssets.map((asset) => {
                ...
                return (
                  <tr key={asset.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className={`p-4 text-right font-mono font-bold text-sm ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
                      {isPositive ? '📈' : '📉'} {Math.abs(change).toFixed(2)}%
                    </td>
                    <td className="p-4 text-right font-mono text-sm text-zinc-300">
                      {fiatSymbol}{formatCompact(asset.volumeUsd24Hr)}
                    </td>
                    <td className="p-4 text-right font-mono text-sm text-cyan-400 font-bold">
                      0.0000%
                    </td>
                    <td className="p-4 text-right font-mono text-sm text-zinc-300">
                      {fiatSymbol}0.0
                    ...
                    <td className="p-4 pr-6 text-right">
                      <Link 
                        href="/trade"
                        className="inline-flex items-center gap-1 text-xs font-bold text-cyan-400 hover:text-cyan-300 bg-cyan-400/10 hover:bg-cyan-400/20 px-3 py-1.5 rounded-full transition-colors border border-cyan-400/20"
                      >
